...
> Date: 2026-06-28 | Last updated: 2026-06-30 (bugs MB-10–MB-15 adicionados; BB-3 marcado parcialmente resolvido)
...
| MB-13 | **`case_14` — Método `add` não existe no runtime de Array** — `Method "add" does not exist nor is supported`. O arquivo `VariablesArray.ps` usa `.add(...)` em variáveis de array, mas `ArrayMethods.php` não implementa esse método. `execute()` em `CaseValidation.php` já tem um `return;` defensivo mas a compilação ainda trava o run completo. Descoberto 2026-06-30. | Developer-Compiler | — | Bloqueia toda a suite quando o run não usa `--from/--to` |
...
| P1-6 | **Named attributes / named arguments on any method call** — `foo(name: value)` syntax for any method. (`points.md` item 5) | Developer-Compiler | — | Very common PHP 8 pattern; high demand from users |
...
| TD-8 | **Deduplicate `instanceof` loops in `Binder.php`** — two `foreach` loops iterate the same `$program->statements` with identical `ClassNode\|InterfaceNode` filter; filter once into `$classLike`. (`refactor.md` §1.2) | Developer-Compiler | — | Trivial deduplication with clear fix |
...
| TD-11 | **Magic number `100` in `TokenManager` → named constant `DEFAULT_TOKEN_WINDOW`** — `getLeftTokens(100)` and `getProcessedTokens(100)` have no explanation. (`refactor.md` §3.2) | Developer-Compiler | — | Magic number; one-minute fix |
...
| NC-12 | **case_60 state** — `points.md` observation 3 flags that case_60 may be blocked by BB-2. Needs a quick check: was the case simplified to avoid arithmetic, or is it currently failing? | Runtime state unknown; needs investigation | QA |

---

**Total items: 0 Blocking Bugs · 15 Minor Bugs (6 open, 9 fixed) · 8 P1 Features · 33 P2 Features · 20 Technical Improvements · 21 Tech Debt · 10 Needs Clarification**
