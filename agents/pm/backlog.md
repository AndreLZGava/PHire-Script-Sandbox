...
> Date: 2026-06-28 | Last updated: 2026-07-09 (MB-10, MB-15, P1-6 removidos; MB-16 corrigido; feature 006 concluída)
...
|---|---|---|
| ~~Blocking bugs resolved (BB-1, BB-2, BB-3)~~ | BB-1, BB-2, BB-3 | ✅ **All fixed** |
...
| # | Item | Owner | Depends on | Reason |
|---|------|-------|------------|--------|
| ~~P1-1~~ | ~~**`BinaryExpressionResolver` in `ReturnContext`**~~ | — | — | ✅ **Fixed 2026-07-09** — implementado pela feature 006. `ReturnContext` agora delega para `ExpressionContext` compartilhado; `BinaryExpressionResolver` reconhece todos os operadores aritméticos e de comparação. Validado por `case_61`. |
...
| ~~P1-3~~ | ~~**`DotResolver` propagate focus in statement/assignment contexts**~~ | — | — | ✅ **Fixed 2026-07-09** — implementado como parte da feature 011 (BB-3 completion). `FunctionCallResolver` agora resolve `this.userMethod()` em qualquer contexto de expressão; `ClassBodyResolver` faz lookAhead para popular o registry de métodos; `FunctionCallContext` fecha corretamente em operadores binários para métodos de usuário. Validado por `case_64` e `case_76`. |
...
| P1-9 | **`exception` keyword — declaração de exceptions customizadas** — não existe forma de declarar uma exception em `.ps`; o único caminho atual é interop com PHP via `external`. Proposta: `exception MyError { String message, Int code }` compilando para `class MyError extends \Exception { ... }` com constructor, getters e `getMessage()`/`getCode()` gerados. Inclui: scanner token `T_EXCEPTION`, `ExceptionNode`, `ExceptionContext`, `ExceptionEmitter`, checker (deve estender Exception), sandbox cases. **🔴 Alta prioridade** — ausência bloqueia qualquer domínio que precise de error handling tipado. | PHireScript Architect | — | Padrão fundamental de OOP; sem isso o usuário cai no PHP puro para qualquer erro customizado |
...
| P1-8 | **Getters/setters exist only at emit time — introduce AST nodes** — `GetterSetterEmitter` synthesises methods directly without creating AST nodes; Binder and Checker are blind to them. A future checker validating getter/setter return types or override rules will require a proper AST node. (`points.md` observation 2) — **⚠️ Prioridade baixíssima**: nenhum checker rule depende desses nós hoje; benefício imediato é zero. Só faz sentido implementar quando uma regra concreta de validação (ex: tipo de retorno do getter, override de getter em subclasse) for necessária. | PHireScript Architect | — | Investimento puramente futuro; sem ganho concreto no estado atual |
...
| ~~TD-19~~ | ~~**`ExpressionContext` — generic reusable context for RHS expressions**~~ | — | — | ✅ **Fixed 2026-07-09** — implementado pela feature 006. `ExpressionContext` compartilhado criado; `ReturnContext` e `AssignmentContext` delegam para ele. Validado por `case_61`–`case_65`. |
...
**Total items: 0 Blocking Bugs (all fixed) · 12 Minor Bugs (4 open, 8 fixed) · 6 P1 Features (1 fixed) · 32 P2 Features · 19 Technical Improvements · 19 Tech Debt · 10 Needs Clarification**
