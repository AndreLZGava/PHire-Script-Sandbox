...
**`Program`**
The root AST node produced by `Parser::parse()`. Contains all top-level statements of a `.phs` file: package declaration, imports, class/interface/trait/type declarations, and loose statements.
...
**`Scanner`**
The lexer at `src/Compiler/Scanner.php`. Converts raw `.phs` source string into a `Token[]` array using regex patterns. Recognizes ~20 token types.
